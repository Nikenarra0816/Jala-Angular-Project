...
import { ProjectLeadUploadDialogComponent } from '@jala-modules/dashboards-project/pages/project-lead/project-lead-upload-dialog/project-lead-upload-dialog.component';
...
export class ProjectLeadComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.projectId = this.activatedRoute.parent.snapshot.paramMap.get( 'id' );
		this.formFilter.valueChanges
			...
			.subscribe( result => {
				...
				// this.dataForChart2 = this.forChart2( allResult );
	...
	/*sortDataChart( data ) {
		...
		newData.label.sort( ( a, b ) => {
			const indexA = data.label.findIndex( val => a === val );
			const indexB = data.label.findIndex( val => b === val );
			const sumA = data.data.reduce( ( aa, bb ) => bb.data[indexA] + aa, 0 );
			const sumB = data.data.reduce( ( aa, bb ) => bb.data[indexB] + aa, 0 );
			if ( sumA > sumB ) {
				return -1;
			}
			if ( sumA < sumB ) {
		...
		newData.label.forEach( ( val ) => {
			const oldIndex = data.label.findIndex( val1 => val === val1 );
			data.data.forEach( ( val1, index ) => {
				if ( val1.data.length === newData.data[index].data.length ) {
					newData.data[index].data = [];
				}
				const oldData = val1.data[oldIndex];
				newData.data[index].data.push( oldData );
			} );
		} );
		return newData;
	}*/
	...
	/*forChart2( data: Lead[] ) {
		...
		if ( Array.isArray( data ) && data.length ) {
			...
			return this.sortDataChart( objChart );
		} else {
			return 'empty';
		}
	}*/
	...
	forChartLine( data: Lead[], dateOpt: 'day' | 'week' | 'month' | 'year' ) {
		...
		if ( Array.isArray( data ) && data.length ) {
			...
			/*objChart.label = data.reduce( ( acc, cur ) => {
				const index = acc.findIndex( val => val === format( parseISO( cur.data.createdAt ), 'dd MMM yyyy' ) );
				if ( index === -1 ) {
					acc.push( format( parseISO( cur.data.createdAt ), 'dd MMM yyyy' ) );
				}
				return acc;
			}, [] );
			objChart.label.forEach( ( value, index ) => {
				data.forEach( value1 => {
					if ( format( parseISO( value1.data.createdAt ), 'dd MMM yyyy' ) === value ) {
						const labelIndex = objChart.data.findIndex( val => val.label === value1.getCategory.name );
						if ( labelIndex !== -1 ) {
							objChart.data[labelIndex].data[index] += 1;
						} else {
	...
	openUpload() {
		const dialogRef = this.dialog.open( ProjectLeadUploadDialogComponent, {
			data: this.projectId,
			disableClose: true
		} );
		dialogRef.afterClosed()
	...
	*/
	...
	ngOnInit() {
		const leadDanBleketekan = this.http.getLeadsByProjectId( this.projectId )
			.pipe(
				untilDestroyed( this ),
				tap( value => {
					...
					// this.dataGroup3 = this.setForFilter4( value );
					// console.log( this.dataGroup3 );
					// this.dataForChart1 = this.forChart1( value );
					// this.dataForChart2 = this.forChart2( value );
		...
		/*.subscribe( value => {
			...
			// this.formFilter.get( 'getSalesOfficer' ).setValue( 1008 );
		} );*/
		forkJoin(
